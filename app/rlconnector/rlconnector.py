...
from app.helpers.helpers import (
    ALIVE_FILENAME, DEBUGGING, MAX_DEAD_TIME, MAX_JOB_RUN_TIME_SECONDS, check_if_alive, get_db, row_to_dict, say_im_alive, say_im_dead)
...
def save_results_design_in_DB(db: Session, job_id,  result):
    ...
    """
    # print(Design)
    print(f"save_intermediate_design_in_cosmo is called. {job_id}")
    say_im_alive(db)
    ...
    # # result_as_string = result2.json()
    # result_as_string  = orjson.dumps(result)
    # print(f"result as string is {result_as_string}")
    crud.create_DesignStatus(db, job_id, progress=1, result = b)
    # cosmo.save_intermediate(job, result)
...
def run_rl_job(job_id):
    ...
    # for i in range(n):
    #     time.sleep(1)
    #     print(f"long job {i} of {n}")
    #     crud.create_DesignStatus(db, job.id, i)
